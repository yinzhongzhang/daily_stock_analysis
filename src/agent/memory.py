...
class AgentMemory:
    ...
    def get_stock_history(
        ...
        try:
            ...
            db = get_db()
            records = db.get_analysis_history(stock_code, limit=limit)
            entries = []
            for r in records:
                entries.append(AnalysisMemoryEntry(
                    stock_code=stock_code,
                    date=str(r.get("date", "")),
                    signal=r.get("decision_type", "hold"),
                    sentiment_score=r.get("sentiment_score", 50),
                    price_at_analysis=r.get("current_price", 0.0),
                    was_correct=r.get("backtest_win") if "backtest_win" in r else None,
                ))
            return entries
